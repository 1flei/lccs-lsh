...
void myMihasher::batchquery(UINT32* results, UINT32* numres, qstat* stats, UINT8* queries, UINT32 numq, int dim1queries)
{
...
void myMihasher::query(int k, UINT32* results, UINT32* numres, UINT8* query_)
{
    // assert(K);
    setK(k);
    chunks_buffer.resize(m);
    res_buffer.resize(K * (D + 1));
    qstat stats;
    query(results, numres, &stats, query_, &chunks_buffer[0], &res_buffer[0]);
}

// Temp variables: chunks, res -- I did not want to malloc inside
// query, so these arrays are passed from outside
void myMihasher::query(UINT32* results, UINT32* numres, qstat* stats, UINT8* query, UINT64* chunks, UINT32* res)
{
                    ...
                    if (size) { // the corresponding bucket is not empty
                        ...
                        for (int c = 0; c < size; c++) {
                            ...
                            if (!counter->get(index)) { // if it is not a duplicate
                                counter->set(index);
								hammd = match(codes[index], query, B_over_8);
...
myMihasher::myMihasher(int _B, int _m)
{
    B = _B;
    B_over_8 = B / 8;
    m = _m;
...
void myMihasher::setK(int _K)
{
    K = _K;
}

myMihasher::~myMihasher()
{
    delete[] xornum;
    delete[] H;
}

void myMihasher::populate(UINT8** _codes, UINT32 _N)
{
    N = _N;
    codes = _codes;
    ...
    counter = std::unique_ptr<bitarray>(new bitarray);
    counter->init(N);
}
