...
def run_mplccs(datasets=datasets, Ls=[100], method='mp_lccs', defaultNPs=[2]):
    curtime = strftime("%m-%d_%H_%M", gmtime())
    def getArgs(ds, L, defaultNP):
        return '-A %s -n %d -q %d -d %d -D %s -Q %s -O %s -G %s -r %d -L %d -p %d --binary_input'%\
            (method, ds.n, ds.qn, ds.d, 
            get_dataset_path(ds), 
            get_query_path(ds), 
            get_output_filename(ds, method, curtime), 
...
if __name__ == '__main__':
    # datasets = [Sift(), Gist(), Glove(), Sift10M()]
    # datasets = [Glove()]
    # datasets = [Trevi()]
    # datasets = [Sift10M()]
    # datasets = [Sift()]
    ...
    # run_lcsb(datasets=datasets, Ls=[8, 16, 32], method='lcsb', step=1)

    # run_mplccs(datasets=datasets, Ls=[8, 16, 32, 64, 128, 256], method='mp_lccs')
    # for p in [4, 8, 16]:
    #     run_mplccs(datasets=datasets, Ls=[128], method='mp_lccs', defaultNP=p)
    # run_mplccs(datasets=datasets, Ls=[8, 16, 32, 64, 128, 256], method='mp_lccs', defaultNPs=[0, 1, 2, 4, 8])

    run_mplsh(datasets=datasets, Ls=[1, 2, 3, 4, 8, 16, 32, 64, 128, 256], rratio=1, Ks=list(range(5, 11)), method='mplsh_lshkit')
