...
bool MP_LCCS_REGISTED = registerCallback("mp_lccs",
    ...
    const auto& fif = [&](){
		auto index = make_unique<Index>(n, d, L, r, p);
        index->build(data);
        return index;
    };
...
bool POLYTOPE_MP_LCCS_REGISTED = registerCallback("polytope_mplccs",
		"n qn d L normalized p dataset_filename queryset_filename ground_truth_filename output_filename", [](){
	using namespace MyCallbackRegister;
	int n = argAs<int>("n");
	int qn = argAs<int>("qn");
	int d = argAs<int>("d");
    ...
    const auto& fif = [&](){
		auto index = make_unique<Index>(n, d, L, p);
        index->build(data);
        return index;
    };
    ...
    const auto& fq = [&](Index &index, int k, int nScans, const float* queryi, MinK_List* list) {
        // int nCandidates = k+K*M;
        const auto& f = [&](int idx){
            float l2dist = calc_angle_normalized(d, data[idx], queryi);
            list->insert(l2dist, idx + 1);
        };
        index.query(nScans, queryi, f);
    };
