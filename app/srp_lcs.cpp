...
int srp_lcslsh(						// cosine smilarity search via sign_lsh
	...
	const Result** ground_truth,				// address of truth set
	const string& outputFilename)			// output folder
{
    const auto fif = [&](){
    ...
    benchmarkMinklist(qn, d, query, ground_truth, outputFilename, fif, fq);
	return 0;
}
...
bool SRP_LCS::registered = MyCallbackRegister::registerCallback("srp_lcs", 
	...
	srp_lcslsh(n, qn, d, K, data, query, ground_truth, output_filename);
});
